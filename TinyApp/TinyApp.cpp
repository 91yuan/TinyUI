	...
	INT       nCmdShow)
{
	...
	for (INT i = 5; i != 0; --i)
	{
	struct value_list *tmp = (struct value_list *)malloc(sizeof(struct value_list));
	tmp->to = i;
	tmp->from = i + 1;
	TRACE("to= %d from= %d\n", tmp->to, tmp->from);
	LIST_ADD_AFTER(&(tmp->children), &(_list.children));
	}
	TRACE("-----------------\n");
	struct value_list* tmp1 = NULL;
	LIST_FOR_EACH_ENTRY(tmp1, &_list.children, struct value_list, children)
	{
	TRACE("to= %d from= %d\n", tmp1->to, tmp1->from);
	}*/
